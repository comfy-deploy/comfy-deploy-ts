...
import { workflowRunsStream } from "comfydeploy/funcs/workflowRunsStream.js";
...
async function run() {
  const res = await workflowRunsStream(comfyDeploy, {
    inputs: {
      "prompt": "A beautiful landscape",
      "seed": 42,
    },
...
import {
  ...
  useWorkflowRunsStreamMutation
} from "comfydeploy/react-query/workflowRunsStream.js";
```
