...
import { deploymentRunsSync } from "comfydeploy/funcs/deploymentRunsSync.js";
...
async function run() {
  const res = await deploymentRunsSync(comfyDeploy, {
    inputs: {
      "prompt": "A beautiful landscape",
      "seed": 42,
    },
...
import {
  ...
  useDeploymentRunsSyncMutation
} from "comfydeploy/react-query/deploymentRunsSync.js";
```
