...
async function run() {
  const result = await comfyDeploy.deployment.getInputDefinition({
    id: "<id>",
  });
...
import { deploymentGetInputDefinition } from "comfydeploy/funcs/deploymentGetInputDefinition.js";
...
async function run() {
  const res = await deploymentGetInputDefinition(comfyDeploy, {
    id: "<id>",
  });
...
```typescript
import { ComfyDeployCore } from "comfydeploy/core.js";
import { deploymentGet } from "comfydeploy/funcs/deploymentGet.js";
...
async function run() {
  const res = await deploymentGet(comfyDeploy, {});
