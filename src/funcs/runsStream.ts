...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateRunStreamRunStreamPostData$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/run/stream")();
  ...
  const context = {
    operationID: "create_run_stream_run_stream_post",
    oAuth2Scopes: [],
    ...
    M.sse(
      ...
      z.instanceof(ReadableStream<Uint8Array>).transform(stream => {
        return new EventStream({
          stream,
          decoder(rawEvent) {
            const schema = components.RunStream$inboundSchema;
